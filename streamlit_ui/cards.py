...
""",
    ...
    if data.get("flights"):
        ...
        render_card_grid(
            data["flights"],
            lambda f: card_html(
                f'✈️ {esc(f.get("airline"))}',
                [
                    f'<b>Flight:</b> {esc(f.get("flight"))}',
                    f'<b>Destination:</b> {esc(f.get("destination"))}',
                    f'<b>Price:</b> ${esc(f.get("price"))}',
                    f'<b>Duration:</b> {esc(f.get("duration"))}',
                    f'<b>Departure:</b> {esc(f.get("departure"))}',
                    f'<b>Arrival:</b> {esc(f.get("arrival"))}',
                ],
            ),
            columns=columns,
        )
    ...
    if data.get("hotels"):
        ...
        render_card_grid(
            ...
            lambda h: card_html(
                f'🏨 {esc(h.get("name"))}',
                [
                    f'<b>Stars:</b> {star_icons(h.get("stars"))}',
                    f'<b>Price per night:</b> ${esc(h.get("price"))}',
                    f'<b>Rating:</b> {esc(h.get("rating"))}',
                ],
    ...
    if data.get("activities"):
        ...
        render_card_grid(
            ...
            lambda a: card_html(
                f'📸 {esc(a.get("name"))}',
                [
                    f'<b>Category:</b> {esc(a.get("category"))}',
                    f'<b>Price:</b> ${esc(a.get("price"))}',
                    f'<b>Duration:</b> {esc(a.get("duration"))}',
                ],
                a.get("suitability") or "",
            ),
            columns=columns,
        )
    ...
    if data.get("restaurants"):
        ...
        render_card_grid(
            ...
            lambda r: card_html(
                f'🍽️ {esc(r.get("name"))}',
                [
                    f'<b>Cuisine:</b> {esc(r.get("cuisine"))}',
                    f'<b>Price level:</b> {esc(r.get("price_level"))}',
                    f'<b>Rating:</b> {esc(r.get("rating"))}',
                ],
                r.get("special_features") or "",
            ),
            columns=columns,
        )
    ...
    if data.get("car_rentals"):
        ...
        render_card_grid(
            ...
            lambda c: card_html(
                f'🚗 {esc(c.get("company"))}',
                [
                    f'<b>Location:</b> {esc(c.get("location"))}',
                    f'<b>Type:</b> {esc(c.get("type"))}',
                    f'<b>Price per day:</b> ${esc(c.get("price"))}',
                    f'<b>Seats:</b> {esc(c.get("seats"))}',
                ],
            ),
            columns=columns,
        )
    ...
    detail_map = [
        ...
        ("seasonal_recommendations", "🌤️ Seasonal notes"),
        ("warning", "⚠️ Warning"),
    ...
    if detail_items:
        ...
        cols = st.columns(detail_columns)
        for idx, (key, title) in enumerate(detail_items):
            with cols[idx % detail_columns]:
